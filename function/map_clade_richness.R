...
# get info from the occ_df ----------------------------------------------

max_richness <- function(occ_df){
  occ_df %>% 
...
xy_limits <- function(occ_df){
  list(x = range(occ_df$x), y = range(occ_df$y))
}


# Generate richness from clade selecteed ----------------------------------

calc_richness <- function(tree_df, occ_df){
  require(dplyr)
  
  
  selected_tips <- tree_df %>% 
    filter(isTip, node_selected) %>% 
    pull(label)
