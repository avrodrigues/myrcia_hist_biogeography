...
for(phy in 1:ncol(evo_mtx)){
  ...
  for(sp in 1:ncol(myrcia_comp)){
    ...
    evo_spp <- evo_mtx[myrcia_comp[[sp]] == 1, phy] 
    
    areas <- table(evo_spp)
    areas_prop <- areas/sum(areas)
    areas_dist <- names(areas_prop)[which(areas_prop > 0.2)]
...
for(i in seq_along(zero200_vec)){
  if(i == 1){
    ...
    filename <- paste0(zero200_vec[i], "_areas_myrcia_phy_consensus.data")
  } else {
    area_mtx <- l_spp_area[[i]]
    filename <- paste0(
